...
class SBModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE','SB', 'KL']
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_KL > 0.0:
            self.loss_KL = self.calculate_KL_loss(self.real_B, fake)
        else:
            self.loss_KL = 0
        
        print(self.loss_KL)
        #import ipdb; ipdb.set_trace()
        self.loss_G = self.loss_G_GAN + self.loss_SB + loss_NCE_both + self.loss_KL
    ...
    def calculate_KL_loss(self, src, tgt):
        #src is true image and tgt is generated image
        f_transform_src = torch.fft.fft2(src)
        f_transform_tgt = torch.fft.fft2(tgt)
        f_transform_shifted_src = torch.fft.fftshift(f_transform_src)
        f_transform_shifted_tgt = torch.fft.fftshift(f_transform_tgt)
        
        # Compute the phase spectrum
        phase_spectrum_src = torch.angle(f_transform_shifted_src)
        phase_spectrum_tgt = torch.angle(f_transform_shifted_tgt)

        cos_diff = torch.cos(phase_spectrum_src - phase_spectrum_tgt)
        numerator = torch.sum(cos_diff)
        denominator = torch.sqrt(torch.sum(torch.cos(phase_spectrum_src)**2) * torch.sum(torch.cos(phase_spectrum_tgt)**2))
